...
def trans_seq(seq):
    ...
    for i in trans_seq:
        trans_seq_list.append(trans_dict[i])
            
    trans_seq_result = "".join(trans_seq_list)
    
    return trans_seq_result
...
def main():
    with open("rosalind_ba1j.txt") as fp:
        result = fp.readlines() 
        seq = result[0].strip()
    ...
    for pattern in patterns:
        patterns_count.append(find_pair(pattern, seq, d) + find_pair(trans_seq(pattern), seq, d))
    
    max_index = [i for i in range(len(patterns_count)) if patterns_count[i] == max(patterns_count)]
    
    for index in max_index:
        print([patterns[index],trans_seq(patterns[index])])
